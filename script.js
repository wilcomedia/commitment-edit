...
async function submitOrder() {
    ...
    try {
        const response = await fetch(WEBHOOK_URL_FINAL_SUBMIT, {
            method: 'POST',
            body: formData,
            mode: 'no-cors' // allow cross-origin request without CORS pre-flight blocking
        });

        /*
         * When mode is "no-cors" the response is opaque and status will be 0.
         * Treat that as a success because we cannot introspect the body/headers.
         */
        if (!(response.ok || response.type === 'opaque')) {
            const errorText = await response.text().catch(() => 'Opaque response');
