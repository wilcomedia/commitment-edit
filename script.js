            ...
            if (retryCount === maxRetries) {
                ...
                if (!WEBHOOK_URL_FINAL_SUBMIT.includes('make.com')) {
                    ...
                    try {
                        ...
                        await fetch(WEBHOOK_URL_FINAL_SUBMIT, {
                            method: 'POST',
                            body: formData,
                            mode: 'no-cors' // Last resort fallback
                        });
                        // If we get here, assume success since we can't check the opaque response
                        closePaymentModal();
                        showThankYouScreen();
                        // Reset form as in the success path
                        form.reset();
                        ...
                        alert(`There was an error submitting your order. Please try again or contact support.\n\nError details: Could not connect to server. Please check your internet connection and try again.`);
                    }
                }
                return;
            }
