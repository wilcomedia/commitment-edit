...
function closePaymentModal() {
    ...
    function initForm() {
        ...
        // Disable image enlargement across ALL viewports
        document.querySelectorAll('.enlarge-icon').forEach(icon => {
            icon.style.display = 'none';
        });

        // Remove any inline onclick handlers that might still trigger enlarge (e.g., jersey swap demo)
        document.querySelectorAll('img[onclick*="showExampleImage"]').forEach(img => {
            img.removeAttribute('onclick');
        });

        // Override global enlarge helpers to no-ops for extra safety
