...
const WEBHOOK_URL_PAYMENT_LINK = 'https://hook.us2.make.com/kvyirmynn7jco8jfbiao62nm1phby9hq'; // Change if a different endpoint generates the payment link
...
function showEnlargedImage(element) {
    ...
    if (enlargedImage && imageModal) {
        ...
        // Use flex layout so we can control alignment easily
        imageModal.style.display = 'flex';
        imageModal.style.justifyContent = 'center';   // horizontal center
        imageModal.style.alignItems = 'center';       // vertical center for edit examples
        imageModal.style.paddingTop = '0';            // reset any CSS offset

        // Ensure the modal content has no extra offset
...
function showExampleImage(imgEl) {
    ...
    if (enlargedImage && imageModal) {
        ...
        imageModal.style.alignItems = 'flex-start';      // start from top, we will offset manually
...
async function submitOrder() {
    ...
    // This is more robust than the 'photos[]' convention.
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(`HTTP error! status: ${response.status}. Server says: ${errorText}`);
        }
...
function closePaymentModal() {
    ...
    function initForm() {
        ...
        // Disable image enlargement on small screens (mobile)
        const isMobileViewport = window.matchMedia('(max-width: 768px)').matches;

        if (!isMobileViewport) {
            // Desktop/tablet → keep enlargement functionality
            ...
            // Mobile → remove/disable enlargement completely
            // 1. Hide any remaining icons (extra safety if CSS hasn't loaded yet)
            document.querySelectorAll('.enlarge-icon').forEach(icon => {
                icon.style.display = 'none';
            });

            // 2. Strip inline onclick handlers that call showExampleImage (e.g., jersey swap demo)
            document.querySelectorAll('img[onclick*="showExampleImage"]').forEach(img => {
                img.removeAttribute('onclick');
            });

            // 3. Override the enlarge helpers with no-ops to guard against stray calls
            window.showEnlargedImage = () => {};
            window.showExampleImage = () => {};
        }
    ...
    // Smooth-scroll so the order summary is roughly centered before showing the modal
    ...
    // Scroll viewport to top to guarantee the modal is fully visible
